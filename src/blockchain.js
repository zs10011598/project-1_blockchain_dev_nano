    ...
    submitStar(address, message, signature, star) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if(currentTime - messageTime > 300){
                reject(new Error('Elapsed time greater than 5 minutes!'));
            }
            if(!bitcoinMessage.verify(message, address, signature)){
                reject(new Error('Bitcoin message not verified!'));   
            }
            let newBlock = new BlockClass.Block({data: star, owner: address});
            self._addBlock(newBlock).then(block => {
                resolve(block);
            }).catch(err => {
                reject(err);
            });
            ...
            try{
                ...
                while(block.height > 0){
                    ...
                    block.validate().then(valid => {
                        if(!valid){
                            errorLog.push(new Error(`Block hash ${block.hash} tampered!`));
                        }
                    });
                    ...
                    if(previousHash != block.hash){
                        errorLog.push(new Error('Chain broken!'));
                    }
